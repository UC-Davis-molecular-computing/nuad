...
NON_CLAMP_POOL: nc.DomainPool = nc.DomainPool('non_clamp_domain_pool', FULL_DOMAIN_LENGTH - CLAMP_LENGTH)
...
design.add_subdomains('y1', [('y1_delta', ALMOST_FULL_DOMAINS_LENGTH), ('y1_toe', TOEHOLD_DOMAIN_LENGTH)])
design.add_subdomains('y1_toe', [('y1_toe_non_clamp', TOEHOLD_DOMAIN_LENGTH - CLAMP_LENGTH), ('clamp', CLAMP_LENGTH)])
design.domains_by_name['y1_toe_non_clamp'].pool = nc.DomainPool(
    'toe_non_clamp_domain_pool', TOEHOLD_DOMAIN_LENGTH - CLAMP_LENGTH
)
# Y2, Y3
#         non_clamp           clamp
# [= = = = = = = = = = = = = | = =>

design.add_subdomains('y2', [('y2_non_clamp', NON_CLAMP_LENGTH), ('clamp', CLAMP_LENGTH)])
design.add_subdomains('y3', [('y3_non_clamp', NON_CLAMP_LENGTH), ('clamp', CLAMP_LENGTH)])
...
fuel_1_bottom = design.add_strand(
    name='fuel_1_bottom', domain_names=[starred_X1, starred_X2, starred_X3, starred_clamp]
)
...
fuel_2_bottom = design.add_strand(
    name='fuel_2_bottom', domain_names=[starred_X2, starred_X3, starred_Y1, starred_clamp]
)
...
fuel_3_bottom = design.add_strand(
    name='fuel_3_bottom', domain_names=[starred_X3, starred_Y1, starred_Y2, starred_clamp]
)
...
reporter_bottom = design.add_strand(name='reporter_bottom', domain_names=[starred_Y1, starred_Y2, starred_Y3])
...
X_complex_constraint = nc.nupack_complex_base_pair_probability_constraint(strand_complexes=[X_complex])
...
# Constraints
constraints = [X_complex_constraint, fuel_complex_constraint, reporter_complex_constraint, four_g_constraint]

params = ns.SearchParameters(  # weigh_violations_equally=True,
    constraints=constraints, out_directory='output/strand_displacement', report_only_violations=False, restart=False
)
