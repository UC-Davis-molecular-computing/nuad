...
def parse_command_line_arguments() -> CLArgs:
    ...
    parser.add_argument(
        "-o", "--output-dir", type=str, default=default_directory, help="directory in which to place output files"
    )

    parser.add_argument("-s", "--seed", type=int, help="seed for random number generator")

    parser.add_argument("-w", "--width", type=int, required=True, help="width of canvas (number of tiles)")

    parser.add_argument("-ht", "--height", type=int, required=True, help="height of canvas (number of tiles)")
...
def create_design(width: int, height: int) -> nc.Design:
    ...
    domain_pool_10 = nc.DomainPool("length-10_domains", 10, numpy_filters=numpy_filters)
    domain_pool_11 = nc.DomainPool("length-11_domains", 11, numpy_filters=numpy_filters)
...
def create_tile_no_gggg_constraint(weight: float) -> nc.StrandConstraint:
    ...
    def evaluate(seqs: tuple[str, ...], _strand: Optional[nc.Strand]) -> nc.Result:  # noqa
        sequence = seqs[0]
        if "GGGG" in sequence:
            result = nc.Result(excess=1.0, summary=f"GGGG found in {sequence}", value=sequence.count("GGGG"))
        else:
            result = nc.Result(excess=0.0, summary="", value=0)
        return result

    description = "No GGGG allowed in strand's sequence"

    return nc.StrandConstraint(description=description, short_description="NoGGGG", weight=weight, evaluate=evaluate)
