...
class CLArgs:
    directory: str
    initial_design_filename: str | None
...
def parse_command_line_arguments() -> CLArgs:
    ...
    parser = argparse.ArgumentParser(  # noqa
        description="Small example design for testing.", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument(
    ...
    return CLArgs(
        ...
        initial_design_filename=args.initial_design,
        weigh_constraint_violations_equally=args.weigh_violations_equally,
...
def main() -> None:
    ...
    strand0: nc.Strand = initial_design.add_strand(["s1", "w1", "n1", "e1"], name="strand 0")
    strand1: nc.Strand = initial_design.add_strand(["s2", "w2", "n2", "e2"], name="strand 1")
    strand2: nc.Strand = initial_design.add_strand(["n2*", "e1*", "n3*", "e3*"], name="strand 2")
    strand3: nc.Strand = initial_design.add_strand(["s4*", "w4*", "s1*", "w2*"], name="strand 3")
