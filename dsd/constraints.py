...
class DomainPool:
        ...
        if self.possible_sequences is not None:
            for idx, seq in enumerate(self.possible_sequences):
                if len(seq) != self.length:
                    raise ValueError(f'ERROR: You specified that DomainPool "{self.name}" has length '
                                     f'{self.length},\nbut an explicitly specified sequence '
                                     f'"{seq}", index {idx} in the list possible_sequences,\nhas length '
                                     f'{len(seq)}.')
    ...
    def to_json_serializable(self, suppress_indent: bool) -> Dict[str, Any]:
        dct = {
            name_key: self.name,
