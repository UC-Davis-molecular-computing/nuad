...
However, the parallelism feature is there if desired, and it is the reason that the `evaluate` function described in `#data-model` above take slightly more complicated arguments than hinted in that section, which we explain next.
